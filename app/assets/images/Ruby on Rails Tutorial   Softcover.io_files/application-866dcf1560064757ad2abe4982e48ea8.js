...
(function() {
  window.Book = {
    ...
    currentScrollTop: 0,
    init: function(opts) {
      var dropdownEls, normalizeChapterNumber;
      this.opts = opts;
      ...
      return $("#sizeBigger").click((function(_this) {
        return function() {
          $('#book').removeClass();
...
(function() {
  ...
  window.setupDropDowns = function() {
    ...
    $('.bookControls').click(function() {
      return $(this).parents('.yourBookItem').removeClass('active');
    });
  ...
  window.gchartAllOne = function() {
    var chartData, chartHeader, chartTable, data, options;
    chartTable = [];
    chartHeader = [];
    chartData = [];
    options = {
    ...
    return new google.visualization.LineChart(document.getElementById('chartWr')).draw(data, options);
  };

  window.gchartMany = function() {
    var chart, chartData, chartHeader, colSize, colVals, colors, count, data, em, en, f, i, options, _results;
    chartHeader = [];
    chartData = [];
    $(".chartTable thead").each(function() {
      $(this).find("th").each(function() {
    ...
    count = chartHeader.length;
    chartData = new Array(count);
    i = 0;
    while (i < count) {
      chartData[i] = [["x"]];
      chartData[i][0].push(chartHeader[i]);
      i++;
    }
    colSize = $(".chartTable tr td:nth-child(1) .datum").map(function() {
      return $(this).html();
    }).get();
    colSize = colSize.length;
    colVals = [];
    i = 0;
    while (i < colSize) {
      colVals[i] = $(".chartTable tr:nth-child(" + (i + 1) + ") .datum").map(function(n) {
        if (n === 0) {
          return $(this).html();
        } else {
          return parseFloat($(this).html().match(/[0-9,.]+/)[0].replace(',', ''));
        }
      }).get();
      en = 1;
      while (en < count + 1) {
        em = [colVals[i][0], parseFloat(colVals[i][en])];
        chartData[en - 1].push(em);
        en++;
      }
      i++;
    }
    ...
    while (i < count) {
      ...
      $("#chartWr").append($("<div/>").attr("id", "chart" + i));
      data = google.visualization.arrayToDataTable(chartData[i]);
      chart = new google.visualization.LineChart(document.getElementById("chart" + i)).draw(data, options);
      _results.push(i++);
    }
    return _results;
  };
